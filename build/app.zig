...
pub const AppOptions = struct {
    ...
    /// Optional app-owned Zig module that customizes the generated desktop
    /// runner for a `src/core.ts` app. The module implements the narrow
    /// `native_extension` contract documented in
    /// `src/app_runner/native_extension.zig`: configure the generated
    /// `TsUiApp` core/options, then optionally wrap its `native_sdk.App`.
    /// Paths are relative to `app_root`. Zig-core apps own their entry point
    /// directly and must leave this null.
    native_extension: ?[]const u8 = null,
};
...
fn tsCoreStage(
    ...
    const core_root = staged.addCopyFile(shim, "core.zig");
...
pub const AppArtifacts = struct {
    ...
    /// The app-owned `native_extension` module behind `exe` and `tests`
    /// (null for Zig-core apps and for TS apps that declared none). A Zig
    /// module may only import files below its own root, and an extension
    /// that fronts a real engine keeps that engine elsewhere in the repo:
    /// the app's build.zig adds those imports here, once per graph, the same
    /// way it already decorates `exe.root_module`.
    extension: ?*std.Build.Module = null,
    test_extension: ?*std.Build.Module = null,
    /// The extension's own test binary. Zig collects tests from the root
    /// module only, never from an imported one, so the extension's tests
    /// cannot ride the app test binary: a guard placed in an extension and
    /// run through `native test` passed while deliberately asserting false.
    /// This artifact is rooted at the extension instead and wired into the
    /// `test` step; its `root_module` is a third instance the app's
    /// build.zig decorates alongside the two above.
    extension_tests: ?*std.Build.Step.Compile = null,
};
...
pub fn addAppArtifacts(b: *std.Build, dep: *std.Build.Dependency, app_options: AppOptions) AppArtifacts {
    ...
    if (app_options.native_extension != null and core_tree != .ts) {
        @panic("\nAppOptions.native_extension extends the generated src/core.ts runner only. Zig-core apps own src/main.zig directly and must leave native_extension null.\n");
    }
    ...
    if (extension_tests) |artifact| test_step.dependOn(&b.addRunArtifact(artifact).step);
    ...
    return .{
        ...
        .install = install,
        .run = run,
        .extension = app_mod.import_table.get("native_extension"),
        .test_extension = test_app_mod.import_table.get("native_extension"),
        .extension_tests = extension_tests,
    };
}

/// A test binary rooted at the app's `native_extension` module, sharing the
/// generated core instance the app test module imports (Model/Msg identity
/// is the extension contract). It links the compiled core archive and the
/// markup data object itself because a root module carries its own link
/// inputs; the app test binary keeps its own copies.
fn extensionTestArtifact(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, app_options: AppOptions, ts_stage: ?TsCoreStage, test_app_mod: *std.Build.Module) ?*std.Build.Step.Compile {
    const path = app_options.native_extension orelse return null;
    const stage = ts_stage orelse return null;
    const native_sdk_mod = test_app_mod.import_table.get("native_sdk") orelse return null;
    const core_mod = test_app_mod.import_table.get("core") orelse return null;
    const root = b.createModule(.{
        .root_source_file = b.path(appPath(b, app_options.app_root, path)),
        .target = target,
        .optimize = optimize,
    });
    root.addImport("native_sdk", native_sdk_mod);
    root.addImport("core", core_mod);
    root.link_libc = true;
...
fn appModule(b: *std.Build, dep: *std.Build.Dependency, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, app_options: AppOptions, manifest_name: []const u8, options_mod: *std.Build.Module, ts_stage: ?TsCoreStage, relational_migrations: std.Build.LazyPath, app_config: AppManifestBuildConfig) *std.Build.Module {
    ...
    if (ts_stage) |stage| {
        // The generated runner and the app-owned extension must see the exact
        // same compiled-core module instance: Model/Msg identity is part of
        // the extension contract, not a structurally similar duplicate.
        const core_mod = b.createModule(.{
            .root_source_file = stage.core_root,
            .target = target,
            .optimize = optimize,
        });
        core_mod.addImport("native_sdk", native_sdk_mod);
        app_mod.addImport("core", core_mod);

        const extension_mod = b.createModule(.{
            .root_source_file = if (app_options.native_extension) |path|
                b.path(appPath(b, app_options.app_root, path))
            else
                dep.path("src/app_runner/native_extension.zig"),
            .target = target,
            .optimize = optimize,
        });
        extension_mod.addImport("native_sdk", native_sdk_mod);
        extension_mod.addImport("core", core_mod);
        app_mod.addImport("native_extension", extension_mod);
