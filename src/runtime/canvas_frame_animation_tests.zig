...
test "runtime stamps zero-start render animations at the first recorded plan" {
    ...
    const preview = try @import("canvas_frame.zig").RuntimeCanvasFrames(@TypeOf(harness.runtime)).planCanvasFrameForView(
        &harness.runtime,
        0,
        .{ .frame_index = 2, .timestamp_ns = 4_500_000_000 },
        frame_storage,
        false,
    );
    try std.testing.expectEqual(@as(f32, 0), preview.render_plan.commands[0].opacity);
    try std.testing.expectEqual(@as(u64, 0), (try harness.runtime.canvasRenderAnimations(1, "canvas"))[0].start_ns);
    ...
    try std.testing.expectEqualDeep(canvas.Affine.identity(), final_frame.render_plan.commands[0].transform);
    try std.testing.expectEqual(@as(usize, 0), (try harness.runtime.canvasRenderAnimations(1, "canvas")).len);
    var loop = animations[0];
    loop.loop = .wrap;
    _ = try harness.runtime.setCanvasRenderAnimations(1, "canvas", &.{loop});
    const loop_frame = try harness.runtime.nextCanvasFrame(1, "canvas", .{
        .frame_index = 5,
        .timestamp_ns = 6_500_000_000,
    }, frame_storage);
    try std.testing.expectEqual(@as(u64, 0), (try harness.runtime.canvasRenderAnimations(1, "canvas"))[0].start_ns);
    try std.testing.expectEqual(@as(f32, 0.5), loop_frame.render_plan.commands[0].opacity);
}
