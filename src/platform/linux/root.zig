...
extern fn native_sdk_gtk_decode_image(bytes: [*]const u8, bytes_len: usize, pixels: [*]u8, pixels_len: usize, max_pixels: usize, out_width: *usize, out_height: *usize) c_int;
...
pub const LinuxPlatform = struct {
    ...
    pub fn deinit(self: *LinuxPlatform) void {
        ...
        if (self.channel_wake_abandoned.load(.seq_cst)) {
            std.debug.print("linux platform teardown: an abandoned platform call may still enter this host; skipping destruction and leaking it (and the wrapper it enters through), process-lived, so the stale call stays safe\n", .{});
            return;
        }
    ...
    fn supportsFeature(context: *anyopaque, feature: platform_mod.PlatformFeature) bool {
        ...
        return switch (feature) {
            ...
            .microphone_capture, .system_audio_capture => false,
            .tray => false,
...
fn decodeImage(context: ?*anyopaque, bytes: []const u8, buffer: []u8, max_pixels: usize) anyerror!platform_mod.DecodedImage {
    ...
    var width: usize = 0;
    var height: usize = 0;
    return switch (native_sdk_gtk_decode_image(bytes.ptr, bytes.len, buffer.ptr, buffer.len, max_pixels, &width, &height)) {
