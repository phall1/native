...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>WebView</td>
      <td><a href="/docs/frontend">Embedded web content</a>: web frontends, documents, previews</td>
      <td><code>ViewKind.webview</code>, <code>WebViewSource</code>, and the selected <a href="/docs/web-engines">web engine</a></td>
    </tr>
  </tbody>
</table>
...
The generated app declares one window with one GPU surface, and the [`UiApp`](/docs/app-model) loop renders the whole UI into it:
...
`gpu_material` is an optional surface property with a default of `none`. Set it to
`glass` only after checking `runtime.supports(.gpu_surface_material)` when the
effect is essential:

```zig
.{ .label = "header-canvas", .kind = .gpu_surface, .fill = true, .gpu_material = .glass }
```

On macOS 26 and later the system host uses the native glass material. Older macOS
system hosts use a semantic visual-effect fallback; other hosts report the feature
unsupported and do not synthesize a fake effect. Material follows the window's
native system appearance, including accessibility appearance choices, rather than
an app token palette.

Glass is visible only through transparent pixels in the GPU canvas. Render opaque
terminal or application content normally: it remains opaque and unchanged above
the material. The registered GPU surface remains the accessibility and input owner;
the host's material and Metal-content views are passive. A visible material surface
temporarily makes its owning window alpha-capable and restores that window's prior
opaque/background state when its final material surface is hidden or closed.

For visual verification, capture the **screen** with a known backdrop behind the
window. Layer-only or window-only captures omit the desktop/backdrop that native
material samples and can look uniformly gray even when composition is correct.

See [Platform Support](/docs/platform-support) for the current host matrix.

## Related Docs

- [App Model](/docs/app-model)
- [Native Controls](/docs/native-controls)
- [Commands](/docs/commands)
- [Capabilities](/docs/capabilities)
- [Multiple WebViews](/docs/webviews)
- [Config](/docs/app-zon)
