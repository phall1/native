...
The default Native SDK app is native-rendered: the view is a Native markup file (`.native`), the logic is an app core — TypeScript by default, or Zig — and rendering happens through Native SDK's own engine. Scaffold one with `native init my_app` and run it with `native dev`.

An app is four things:

- `src/app.native` — the entire UI: elements, layout, bindings, and message dispatch.
- `Model` — a plain data structure holding your state.
...
| `media-surface` | media surface | composites a texture produced OUTSIDE the widget tree (video, camera, an external renderer) like any widget; `surface="{binding}"` (required) binds the model-owned `u64` surface id a Zig-tier producer targets — no intrinsic size, display-only, `label` it; goldens and replay see the deterministic id-derived placeholder, never producer frames — see [Media Surface](/docs/components/media-surface) and [Media Producers](/docs/media-producers) |
| `video` | video playback | leaf; `src` declares the app's single platform-decoded playback (an assets path or `http(s)` URL, local-first like audio), `autoplay`/`loop`/`muted` shape it, and `controls` composes the house transport chrome — without it the element is the surface alone and your update code drives the video command vocabulary; no intrinsic size, `label` it; goldens and replay see the placeholder, transport is journaled truth — see [Video](/docs/components/video) |
...
| `segmented-control` | exclusive-choice control | standalone text-bearing segment; bind `selected`, dispatch `on-press`, and optionally add a vector `icon`; compose several in a row, or use [tabs](/docs/components/tabs) for the strip-and-panel pattern |
...
A few widget capabilities remain proposals rather than markup admissions. `data_grid` still needs a per-column cell-template contract (row type, column value, keys, accessibility, and virtualization behavior); `popover` and `menu_surface` need an anchoring ruling (nearest positioned ancestor versus an explicit anchor relationship, including the HTML-markup surface); and image-backed `icon_button`/`image` usage needs the TypeScript asset-lifecycle ruling. Per-state styling beyond design tokens belongs to the HTML-markup lane's stage 4. Existing vector icon buttons are fully declarative as `<button size="icon" icon="..." label="..." on-press="..." />`; `segmented-control` is a first-class markup leaf, and runtime image bindings remain available on `avatar` and `image` as documented below.
...
`on-press`, `on-toggle`, `on-change`, and `on-submit` name a variant of your `Msg` union, optionally with one binding payload (`toggle:{t.id}`). `on-input` names a variant carrying the runtime's text-input event, and each core tier declares that in its own vocabulary. In a TypeScript core the event union mirrors the runtime's event vocabulary structurally, and `on-input` matches the shape and translates each runtime event into your union at dispatch — import it from `@native-sdk/core/text` (or `@native-sdk/core/events`), or declare the same shape in-file; your `update` reduces the events over the draft bytes. In a Zig core the payload is `canvas.TextInputEvent`; pair it with `canvas.TextBuffer` in the model for editor state that survives rebuilds — the markup binds a `pub fn` returning the text (`text="{draft}"`), and binding the `TextBuffer` field itself directly is a teaching error pointing at this pattern:

<CodeToggle>

```ts
import { type TextInputEvent } from "@native-sdk/core/text";
// insert_text / delete_* / clear / move_caret / set_selection /
// set_composition / commit_composition / cancel_composition — the full
// union is the SDK's; the same shape declared in-file matches identically.
...
`on-scroll` (on the `scroll` element; `Ui.scrollMsg(.tag)` on `on_scroll` in Zig views) names a variant whose payload is `canvas.ScrollState` (in a TypeScript core, the `ScrollState` record from `@native-sdk/core/events`, or the same four-numeric-field shape declared in-file): after every user scroll — wheel, kinetic momentum, keyboard, accessibility — the runtime delivers the post-scroll `offset`, `viewport_extent`, and `content_extent`, so long content can page or lazy-load a bounded window that follows the scrollbar. The payload carries the offset the runtime already applied: store it in the model and echo it back through the scroll's `value`, and rebuilds never fight live scrolling.
...
`on-double-press` in markup (`ElementOptions.on_double_press` in Zig views) names the Msg for a double click. The runtime derives the click count itself, identically on every platform: a primary press within 500 ms and 4 points of the previous one raises the count (clamped at 3 — a fourth rapid click repeats the triple behavior), anything else restarts at 1, and the count rides the gesture to its release. The firing order is additive, never a delay: the first click of a double dispatches `on-press` on its own release as usual, and the second release — click count 2 — dispatches `on-double-press` in place of a second press (a triple click's third release resolves the double handler again). That makes select-on-press + act-on-double-press the natural pairing — a track list where a click selects the row and a double click plays it, with no press timer and no swallowed first click; `examples/code-editor` uses the pair to preview files on click and pin their tabs on double click. A double click on a widget that binds no double handler behaves exactly like two single clicks, and like `on-press`, binding it makes the element a hit target.
...
For anything asynchronous — running `gh`, streaming an agent's stdout — `update` never performs the work; it describes it, and results come back as ordinary Msgs. In a TypeScript core the description is `Cmd` data returned beside the next model ([TypeScript Cores: Effects](/docs/typescript#effects-are-cmd-data) has the whole vocabulary); in a Zig core, `update` takes a third parameter, the effects channel — declare `.update_fx` instead of `.update` (two-argument apps are unchanged). The subprocess stream in both:

<CodeToggle>

```ts
case "start":
  return [model, Cmd.spawn(
    [asciiBytes("gh"), asciiBytes("issue"), asciiBytes("list")],
    {
      key: "issues",
      line: "line",   // each stdout line dispatches { kind: "line", text: Uint8Array }
      exit: "exited", // a clean exit dispatches { kind: "exited", code: number }
      err: "stream_failed",
    },
  )];
case "cancel":
  return [model, Cmd.cancel("issues")]; // the stream's err arm gets `cancelled`
case "line":
  return { ...model, lines: [...model.lines, { text: msg.text }] };
```
...
HTTP requests ride the same channel: one buffered request on a worker thread, delivering its terminal outcome as exactly one Msg — a real response (any status, non-2xx included), a classified failure (`rejected`, `connect_failed`, `tls_failed`, `protocol_failed`, `timed_out`), or `cancelled`:
...
case "load":
  return [model, Cmd.fetch(
    { url: asciiBytes("https://api.example.com/search?q=zig") },
    { key: "search", ok: "fetched", err: "fetch_failed" },
  )];
case "stop":
  return [model, Cmd.cancel("search")];
case "fetched":
  // the ok arm carries { status: number, body: Uint8Array } — a 404 is
  // still ok: an HTTP-level error is a delivered response
  return { ...model, status: msg.status, body: msg.body };
```
...
Source presentation is the reusable `ui.code` / `<code>` component. It is deliberately bare: no background, border, radius, shadow, or padding. Wrap it in a `panel`, `card`, or another container when the design needs surface chrome. It wraps by default; `wrap="false"` preserves logical lines inside one horizontal scroll region, and `line-numbers` opts into a muted logical-line gutter. A definite `height` keeps overflow reachable with vertical scrolling (or both axes when wrapping is off):
...
For example, an inset code well is ordinary composition:
...
The series set is static — the plot's shape is part of the design and the DATA varies through the bindings. `ui.chart` (the same code the markup lowers through) is the escape hatch for what the grammar deliberately excludes: `.band` envelopes (a min/max band pairs a second lower-edge slice per point) and series sets composed dynamically at view time. See [Chart](/docs/components/chart) for the full attribute tables.
...
- The automation harness drives widgets and reads accessibility snapshots and screenshots headlessly — see [Automation](/docs/automation).
