...
- **Pinned macOS host packages.** The scriptc 0.1.1 npm release publishes the compiler and runtime packages needed for Apple-silicon macOS source builds. Although its compiler metadata also names Darwin x64 optional packages, the `@scriptc/llvm-darwin-x64@0.1.1` and `@scriptc/runtime-darwin-x64@0.1.1` registry artifacts are not published, so Intel-macOS source installs are not supported by this pin. This limitation is specific to the macOS host packages in this release; it does not narrow the target matrix above or make a broader claim about other host platforms.
